...
namespace my
{
	...
	void BaseBullet::Render(HDC hdc)
	{
		...
		TransparentBlt(hdc, pos.x, pos.y, Bullet->GetWidth() * 1.8, Bullet->GetHeight() * 1.7, Bullet->GetHdc(), 0, 0,
		...
		if (BulletColl)
		{
			...
			TransparentBlt(hdc, pos.x, pos.y, D_effect->GetWidth() * 3, D_effect->GetHeight() * 3, D_effect->GetHdc(), 0, 0,
