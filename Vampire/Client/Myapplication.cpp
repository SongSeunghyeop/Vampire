...
namespace my 
{
	...
	void Application::Initialize(HWND hwnd)
	{
		...
		RECT rect = { 0,0,1500,750 }; // 윈도우 전체 크기의 사각형

		AdjustWindowRect(&rect, WS_OVERLAPPEDWINDOW, false); // 해상도를 입력하면, 그에 맞게 수정
		SetWindowPos(mhwnd, NULL, 10,0, (rect.right - rect.left), (rect.bottom - rect.top), 0); // 윈도우 크기 및 위치 조정
	...
	void Application::Render()
	{
		...
		StretchBlt(mhdc, 0, 0, 1500,750, backHdc, 0, 0,1500,750, SRCCOPY); // 구현용 hdc에 그대로 복사
	}
}